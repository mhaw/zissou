...
def _doc_to_smart_bucket(doc) -> SmartBucket:
    ...
    filtered_data = {k: v for k, v in smart_bucket_data.items() if k in smart_bucket_fields}
...
def list_smart_buckets() -> list[SmartBucket]:
    ...
    _require_db()
    try:
        smart_buckets_ref = db.collection(os.getenv("FIRESTORE_COLLECTION_SMART_BUCKETS"))
        docs = smart_buckets_ref.stream()
        return [_doc_to_smart_bucket(doc) for doc in docs]
    except GoogleCloudError as e:
        logger.error(f"Firestore error listing smart buckets: {e}")
...
def create_smart_bucket(smart_bucket: SmartBucket) -> str:
    ...
    _require_db()
    try:
        smart_bucket_ref = db.collection(os.getenv("FIRESTORE_COLLECTION_SMART_BUCKETS")).document()
        ...
        logger.error(f"Firestore error creating smart bucket: {e}")
...
def update_smart_bucket(smart_bucket_id: str, update_data: dict):
    ...
    _require_db()
    try:
        smart_bucket_ref = db.collection(os.getenv("FIRESTORE_COLLECTION_SMART_BUCKETS")).document(smart_bucket_id)
        update_data["updatedAt"] = datetime.utcnow()
        if "rules" in update_data:
            update_data["rules"] = [rule.__dict__ for rule in update_data["rules"]]
        smart_bucket_ref.update(update_data)
    except GoogleCloudError as e:
        logger.error(f"Firestore error updating smart bucket {smart_bucket_id}: {e}")
        raise FirestoreError(f"Failed to update smart bucket {smart_bucket_id} in Firestore.") from e
